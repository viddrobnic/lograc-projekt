-- This module contains examples that work with raw 2-3Trees
module ExamplesRaw where
...
open import 23Tree orderedℕ
open import Search orderedℕ
open import Insert orderedℕ

orderedℕ∞ = OrderedSet.S (orderedInfinity orderedℕ)

-- Empty
emptyTree1 : 2-3Tree 0 -∞ +∞
emptyTree1 = Empty -∞ +∞ -∞<+∞

-- Example 2-3 tree.
sampleTree : 2-3Tree 1 -∞ +∞
sampleTree = 2Node 1 -∞<n n<+∞ (Empty -∞ [ 1 ] -∞<n) (Empty [ 1 ] +∞ n<+∞)

sampleTree2 : 2-3Tree 1 -∞ +∞
sampleTree2 = 3Node 1 2 -∞<n (m<n (s≤s (s≤s z≤n))) n<+∞ (Empty -∞ [ 1 ] -∞<n) (Empty [ 1 ] [ 2 ] (m<n (s≤s (s≤s z≤n)))) (Empty [ 2 ] +∞ n<+∞)

sampleTree3 : 2-3Tree 2 -∞ +∞
sampleTree3 = 2Node 3 -∞<n n<+∞ 
  (3Node 1 2 -∞<n 
    (m<n (s≤s (s≤s z≤n))) 
    (m<n (s≤s (s≤s (s≤s z≤n)))) 
    (Empty -∞ [ 1 ] -∞<n) 
    (Empty [ 1 ] [ 2 ] (m<n (s≤s (s≤s z≤n)))) 
    (Empty [ 2 ] [ 3 ] (m<n (s≤s (s≤s (s≤s z≤n)))))) 
  (2Node 4 
    (m<n (s≤s (s≤s (s≤s (s≤s z≤n))))) 
    n<+∞ 
    (Empty [ 3 ] [ 4 ] (m<n (s≤s (s≤s (s≤s (s≤s z≤n)))))) 
    (Empty [ 4 ] +∞ n<+∞))

sampleTree4 : 2-3Tree 2 -∞ +∞
sampleTree4 = 3Node 2 4 -∞<n 
  ...
  (2Node 3 
    ...
    (Empty [ 3 ] [ 4 ] (m<n (s≤s (s≤s (s≤s (s≤s z≤n))))))) 
  (2Node 5 
    (m<n (s≤s (s≤s (s≤s (s≤s (s≤s z≤n)))))) 
    n<+∞ 
    (Empty [ 4 ] [ 5 ] (m<n (s≤s (s≤s (s≤s (s≤s (s≤s z≤n))))))) 
    (Empty [ 5 ] +∞ n<+∞))
